<template>
  <div id="list" style="margin-top: 20px;" class="list_box">
    ...
    <div v-if="showDiv == 2" ref="className" class="box2">
      <el-table :data="tempListArr" style="width: 100%">
        ...
        <el-table-column width="60px" label="选择">
          <template slot-scope="props">
            <input
              type="checkbox"
              v-if="props.row.status == 0 && props.row.isrelease == 1 && props.row.isreg == 1"
              name="checkbox1"
              v-model="checkVal"
              :value="props.row.mid"
              class="checkInput"
            ...
            <span v-if="props.row.status == 0 && props.row.isrelease == 1 && props.row.isreg == 1" ref="radios">
              <span style="margin-right: 10px;">
                <input
                  type="checkbox"
                  :value="1"
                  :data-mid="props.row.mid"
                  :class="'radiosTwo radiosTwo' + props.row.mid"
                  @click="radiosCheck(1, 'radiosTwo' + props.row.mid)"
              ...
              <span>
                <input
                  ...
                  :data-mid="props.row.mid"
                  :class="'radiosTwo radiosTwo' + props.row.mid"
                  @click="radiosCheck(2, 'radiosTwo' + props.row.mid)"
                  style="position: relative;top: 3px;"
                />
            ...
            <el-form label-position="left" inline class="demo-table-expand">
                ...
                <div class="grid-content bg-purple">
                  <el-form-item label="联系记录证明:">
                    <div style="margin-top: 15px;">
                      <div class="img_div_box" v-for="item in props.row.pics"><img @click="getImgSrc(baseURL + '/' + item)" :src="baseURL + '/' + item" style="width: 100%;height: 100%;" alt="" /></div>
                    </div>
                  </el-form-item>
                </div>
              </el-col>
              <el-col :span="24">
                <div style="font-size: 12px;border: 1px blue dashed;border-radius: 6px;padding: 20px;overflow: hidden;">
                  <el-col :span="6">
                    注册是否通过： {{ props.row.isreg == 0 ? '否' : '是' }}
                    <span style="margin-left: 10px;cursor: pointer;color: #2863FD;">查看</span>
                  </el-col>
                  <el-col :span="6">
                    产品发布是否达标： {{ props.row.isrelease == 0 ? '否' : '是' }}
                    <span style="margin-left: 10px;cursor: pointer;color: #2863FD;">查看</span>
                  </el-col>
                </div>
              </el-col>
      ...
      <div style="margin: 20px 20px;overflow: hidden;">
        <div style="float: left;">
          <span>
            <input style="position: relative;top: 2px;" type="checkbox" @click="allCheck" />
            全选
          </span>
          <span></span>
        ...
        <div style="float: right;">
          已选
          <el-button style="margin: 0 10px;" type="primary" size="mini" @click="allBtn">一键提交</el-button>
        </div>
      </div>
    </div>
    <!-- 提现申请列表 -->
    ...
    <!-- 查看联系证明 -->
    <el-dialog :visible.sync="prove" center>
      <img style="height: 100%;width: 100%;" :src="proveSrc" alt="" />
    </el-dialog>
    <!-- 查看凭证 -->
    ...
    <div v-if="showDiv == 5" class="box5">
      <el-table :data="tempListArr" style="width: 100%">
        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-table :data="props.row.descdata" style="width: 100%">
              <el-table-column prop="date" label="日期" width="180"></el-table-column>
              <el-table-column prop="byljtg" label="本月累计推广" width="180"></el-table-column>
              <el-table-column prop="byyxtg" label="本月有效推广"></el-table-column>
              <el-table-column prop="byjxjs" label="本月绩效基数" width="180"></el-table-column>
              <el-table-column prop="bytxje" label="本月提现金额"></el-table-column>
            </el-table>
          </template>
        </el-table-column>
      </el-table>
      <div style="float: right;margin: 20px 30px;"><el-button @click="exportExcel" type="primary">导出Excel</el-button></div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    exportExcel() {
      ...
      window.location.href =
        'https://vx.eaglesell.com.cn/index.php/admin/Publicoperate/excel?number=' +
        number +
        '&name=' +
        name +
        '&sort=' +
